...
1. O frontend “lembra” de reenviar o token em cada requisição depois do login, pois foi utilizado o localStorage do navegador junto com um arquivo que centraliza as chamadas HTTP (api.js). Com isso, quando o usuário se autentica com sucesso no index.html, o backend devolve o token JWT. O arquivo login.js recebe esse valor e o grava no navegador chamando API.setToken(token), que executa localStorage.setItem('jwt_token', token). Então, sempre que qualquer tela precisa fazer uma requisição, ela chama a função request, que verifica se há um token guardado (getToken()). Se sim, ela adiciona o cabeçalho HTTP "Authorization: Bearer token" na requisição. Dessa forma, nenhuma tela precisa se preocupar em formatar o token manualmente.
